...
live on Cloudflare (Pages + D1) for a shared multi-user deployment. Both
...
- **Live/Cloudflare backend:** Pages Functions + D1 (`functions/`, `d1/`)
...
needed) to host this on Cloudflare Pages with D1 as the database and
...
npm run pages:dev        # builds the frontend and serves it + the API together via Wrangler
```
...
functions/   Pages Functions API (D1-backed) — used for the Cloudflare deployment
